...
var _util_colorSVG = require('../_util/colorSVG.js');
...
var Checkbox = React__default["default"].memo((props) => {
    const id = props.id || _util_generatedId();
...
const Checkbox$1 = styled__default["default"].div `
  ...
  & > label:first-of-type {
    ...
    &::after {
      ...
      ${css_base__zIndex.front_1};
      ${(props) => props.color && _util_colorSVG(props.color)}
    }
    i {
      position: absolute;
      width: 5px;
  ...
  & > label:last-of-type {
    margin-left: 5px;
    ${({ color }) => color && _util_colorSVG(color)};
  }
