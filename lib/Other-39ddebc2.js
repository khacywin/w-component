...
var useIntersectionObserver = require('./useIntersectionObserver-efab8f32.js');
...
function Other({ fnChange, format = "DD MMMM,YYYY", label, name, picker = "date-time", style, value = "", isRemove = true, defaultValue, disableItem, position, }) {
    ...
    useIntersectionObserver.useMutationObservable(refMenuDropdown.current, handleDropdownChange);
    // Handle show dropdown menu
    ...
    const containers = React.useMemo(() => new Map()
        ...
        React__default["default"].createElement(useIntersectionObserver.Year, { selected: val, disableItem: disableItem, fnSelected: onChange("year") })))
        .set("month", React__default["default"].createElement(InputDateSelectorContainer, { show: true },
        React__default["default"].createElement(useIntersectionObserver.Month, { selected: new Date(date), disableItem: disableItem, fnSelected: onChange("month") }))), [val, disableItem, onChange, time, date]);
    // Handle value and format data
    // Set state of component with value
    React.useEffect(() => {
        if (!value && !defaultValue)
