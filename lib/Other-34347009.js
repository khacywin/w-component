...
var usePositionDropdown = require('./usePositionDropdown-f98e8ffb.js');
...
function Other({ fnChange, format = "DD MMMM,YYYY", label, name, picker = "date-time", style, value = "", isRemove = true, defaultValue, disableItem, position, }) {
    ...
    const onChange = React.useCallback((field) => (val) => {
        switch (field) {
            case "date": {
                setDate(new Date(usePositionDropdown.dayjs_min(val).format("YYYY/MM/DD") + (time ? " " + time : "")).toString());
                setVal(usePositionDropdown.dayjs_min(val).format(format));
                fnChange === null || fnChange === void 0 ? void 0 : fnChange(usePositionDropdown.dayjs_min(val).format(format));
                break;
            }
            case "month": {
                setDate(new Date(usePositionDropdown.dayjs_min(val).format("YYYY/MM/DD")));
                setVal(usePositionDropdown.dayjs_min(val).format(format));
                fnChange === null || fnChange === void 0 ? void 0 : fnChange(usePositionDropdown.dayjs_min(val).format(format));
                break;
            }
    ...
    React.useEffect(() => {
        ...
        if (picker === "date-time" || picker === "date") {
            const _date = new Date(_val);
            setDate(_date);
            setTime(usePositionDropdown.dayjs_min(_date).format("hh:mm"));
            setVal(usePositionDropdown.dayjs_min(_date).format(format));
        }
        else if (picker === "year") {
            setVal(new Date(_val.toString()).getFullYear());
        }
        else if (picker === "month") {
            setVal(usePositionDropdown.dayjs_min(_val).format(format));
