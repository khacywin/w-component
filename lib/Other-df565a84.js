...
var useMutationObserver = require('./useMutationObserver-f30dfa92.js');
...
var DateTime = React__default["default"].memo(({ fnChange, format = "DD MMMM,YYYY", label, name, style, value = "", isRemove = true, defaultValue, disableItem, position }) => {
    ...
    return (React__default["default"].createElement(Wrap$1, { ref: refDropdown },
        React__default["default"].createElement(_FormGroup.FormGroup, { style: style || {}, isFocus: true, label: label, id: id },
            ...
            React__default["default"].createElement(DropdownMenu$1, { position: position, ref: refMenuDropdown, show: isDisplay },
...
function Other({ fnChange, format = "DD MMMM,YYYY", label, name, picker = "date-time", style, value = "", isRemove = true, defaultValue, disableItem, position, }) {
    ...
    const handleDropdownChange = React.useCallback((mutationList) => {
        console.log({
            mutationList,
        });
        handlePosition(picker === "time" ? { left: widthInputDate } : {});
    }, [handlePosition, picker, widthInputDate]);
    useMutationObserver.useMutationObservable(refMenuDropdown.current, handleDropdownChange);
    // Handle show dropdown menu
    ...
    const containers = React.useMemo(() => new Map()
        ...
        React__default["default"].createElement(useMutationObserver.Year, { selected: val, disableItem: disableItem, fnSelected: onChange("year") })))
        .set("month", React__default["default"].createElement(InputDateSelectorContainer, { show: true },
        React__default["default"].createElement(useMutationObserver.Month, { selected: new Date(date), disableItem: disableItem, fnSelected: onChange("month") }))), [val, disableItem, onChange, time, date]);
    // Handle value and format data
    // Set state of component with value
    React.useEffect(() => {
        if (!value && !defaultValue)
