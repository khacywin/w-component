...
var index$6 = require('./index-222b36c6.js');
...
function DateRange({ fnChange, label, picker = "date", value = {}, defaultValue = {}, }) {
    ...
    const disablePrevious = React.useCallback((date) => {
        const _date = weekOfYear.dayjs_min(date);
        return value.from
            ? _date.isBefore(weekOfYear.dayjs_min(value.from))
            : defaultValue.from
                ? _date.isBefore(weekOfYear.dayjs_min(defaultValue.from))
                : false;
    }, [defaultValue.from, value.from]);
    const disableNext = React.useCallback((date) => {
        const _date = weekOfYear.dayjs_min(date);
        return value.to
            ? _date.isAfter(weekOfYear.dayjs_min(value.to))
            : defaultValue.to
                ? _date.isAfter(weekOfYear.dayjs_min(defaultValue.to))
                : false;
