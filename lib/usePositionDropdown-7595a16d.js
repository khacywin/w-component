...
function usePositionDropdown(ref) {
    ...
    const _handlePosition = React.useCallback((add) => {
        ...
        const newRex = /(-?[0-9\.]+)/g;
        const translateValue = window
            .getComputedStyle(ref.current)
            .getPropertyValue('transform')
            .match(newRex);
        let transformOriginX = 'left';
        let transformOriginY = 'top';
        let translateX = +(translateValue === null || translateValue === void 0 ? void 0 : translateValue[4]) || 0, translateY = +(translateValue === null || translateValue === void 0 ? void 0 : translateValue[5]) || 0;
        if (add) {
        ...
        if (posElement.x < 0) {
            translateX += +posElement.x;
            transformOriginX = 'left';
        }
        else if (posElement.right > pScreen.width) {
            translateX += -(posElement.right - pScreen.width);
            transformOriginX = 'right';
        }
        if (posElement.top < 0) {
            translateY += -posElement.height;
            transformOriginY = 'top';
        }
        else if (posElement.bottom > pScreen.height) {
            transformOriginY = 'bottom';
            translateY += -(posElement.bottom - pScreen.height);
        }
