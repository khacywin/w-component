import { borderRadius, fontSize, fontWeight, space } from "components/util/css/base";
...
export default function ({ amount, sum, title }: ChartProcessProps) {
  let percent = (amount / sum) * 100;
  return (
    <>
      <WrapChartProcess sum={sum}>
        <ChartProcess amount={amount} percent={percent} />
      </WrapChartProcess>
      {title ? <ChartProcessTitle>{title}</ChartProcessTitle> : ""}
    </>
  );
}
