  ...
  ({
    ...
    const pushIntoArray = useCallback(() => {
      ...
      let index = 1,
        indexNext = 1,
        indexPrev: number =
          wdate.numberOfDays(wdate.previousMonth(select)) - firstDay + 1;
      for (let row = 0; row < 7; row++) {
        for (let col = 0; col < 6; col++) {
          ...
          if (row === 0 && col < firstDay) {
            ...
            html.push(
              <td
                className={`w-calendar-date not-in-month
                ${
                ...
                ${disableItem?.(_dateString) ? "disable" : ""}
                `}
                onClick={selectDate}
    ...
    return (
      <div className={`w-calendar ${noSelect ? "w-calendar-no-select" : ""}`}>
